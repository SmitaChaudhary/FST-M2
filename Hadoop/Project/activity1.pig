-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/smita/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/smita/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/smita/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/smita/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/smita/outputs' USING PigStorage('\t');
